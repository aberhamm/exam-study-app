/**
 * Create/Update Atlas Vector Search Index for competency embeddings
 *
 * Usage:
 *   tsx scripts/create-competencies-vector-index.ts [--index <name>] [--dims <n>] [--similarity <cosine|euclidean|dotProduct>] [--update]
 *   env: MONGODB_URI, MONGODB_DB, MONGODB_EXAM_COMPETENCIES_COLLECTION, MONGODB_COMPETENCIES_VECTOR_INDEX, QUESTIONS_EMBEDDING_DIMENSIONS
 */
...
function parseArgs(): Args {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (a === '--index') out.indexName = args[++i];
      ...
      out.similarity = args[++i] as 'cosine' | 'euclidean' | 'dotProduct';
    else if (a === '--update') out.update = true;
    else if (a === '--help' || a === '-h') {
      console.log(
        'Usage: tsx scripts/create-competencies-vector-index.ts [--index <name>] [--dims <n>] [--similarity <cosine|euclidean|dotProduct>] [--update]'
      );
      process.exit(0);
    }
  }
...
async function main() {
  ...
  const collection = envConfig.mongo.examCompetenciesCollection;
  const indexName = cliIndex || envConfig.mongo.competenciesVectorIndex;
  const dims =
    typeof cliDims === 'number' && !Number.isNaN(cliDims)
      ? cliDims
      : (() => {
...
`);
    ...
    try {
      ...
      console.warn(
        'Could not list existing search indexes (ensure Atlas Search is enabled). Proceeding…'
      );
    }
